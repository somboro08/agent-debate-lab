...
import { getProjectHistory, getProjectState, listProjects, saveDebateSession, saveSessionState } from "./db";
...
export const appRouter = router({
  ...
  debate: router({
    ...
    state: publicProcedure.input(z.object({ sessionKey: z.string().min(1) })).query(({ input }) => getProjectState(input.sessionKey)),
    saveState: publicProcedure.input(z.object({ sessionKey: z.string().min(1), name: z.string(), context: z.string(), status: z.enum(["draft", "running", "paused", "interrupted", "completed"]), activeObjective: z.number().int().nonnegative(), agents: z.unknown().optional(), transcript: z.unknown().optional(), finale: z.unknown().optional() })).mutation(({ input }) => saveSessionState(input)),
